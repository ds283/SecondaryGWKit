...
| 06a | Close-out and the provenance note | [`prompt-06a.md`](prompt-06a.md) | **yes** | The campaign's named deliverable, and the only prompt whose acceptance condition cannot be met by diligence: where a constant's provenance is not in the record the note must say so **in those words**, and a fluent paragraph is exactly what failure looks like. It assembles from **eleven** logs and **five** campaign documents against §5.4's **44 rows** — not the ten, four and "39 plus six" README §3.6a forecasts, all three of which 06 made stale. Closes the campaign |
