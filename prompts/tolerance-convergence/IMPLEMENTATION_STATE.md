...
**Last updated:** 2026-09-18 · **Status: CLOSED — 6 / 6 measured, plus the insertions 02a, 04b, 05b, 05a, 06 and 06a; every board item is landed.** **04b has landed the block prompt 04 stopped short of writing**, **05 has landed the schema change**, **05b has made the recorded order the order that was used**, **05a has decoupled the tolerances that are real**, **06 has measured `QuadSourceIntegral` read-only and regenerated the inventory**, and **06a has written the close-out document and `docs/TOLERANCE-PROVENANCE.md`** — the campaign's durable deliverable (README §1.2), covering all 44 rows of `TOLERANCE-INVENTORY.md` §5.4. **§7 D11 split §3.6 on 2026-09-18**: **06** took the `QuadSourceIntegral` measurement and the inventory regeneration, **T11**; **06a** took the close-out document and the provenance note, **T12**. **This is the last prompt of the campaign; there is no successor board to hand work to** — what remains open is recorded in `docs/OPEN_ISSUES.md` §1.5 and in `docs/TOLERANCE-CONVERGENCE.md` §6.
...
**Prompt 05a has decoupled the tolerances that are real (2026-09-18, README §7 D1 and D9).** The
three targets whose `(atol, rtol)` reaches a solver each carry their own pair, with the measurement
that chose it beside the value in `config/defaults.py`, and `QuadSourceIntegral` — decoupled since
`source-remediation` — is **guarded for the first time**. Six constants ship, every one settled by
the user under D1 on 2026-09-17: `DEFAULT_HEXIT_ABS_TOLERANCE = 1e-10` /
`DEFAULT_HEXIT_REL_TOLERANCE = 1e-9`, `DEFAULT_GK_NUMERIC_ABS_TOLERANCE = 1e-10` /
`DEFAULT_GK_NUMERIC_REL_TOLERANCE = 1e-8`, `DEFAULT_TK_NUMERIC_ABS_TOLERANCE = 1e-13` (already
existed) / `DEFAULT_TK_NUMERIC_REL_TOLERANCE = 3e-11`. **Two values move** — the exit time's `rtol`
and the $T_k$ numeric's, both from `1e-8` — and **after this commit no object type keys on
`DEFAULT_ABS_TOLERANCE` or `DEFAULT_REL_TOLERANCE`**, whose values are unchanged and whose names
survive: what is left of them is seven float comparisons in modules 05a may not edit, so renaming
would reach every one of them and `[02-shared-atol-doubles-as-a-float-comparison-epsilon]` stays
open (log 05a §2 is the §3 decision and its argument). **11 `main.py` sites** moved, `QuadSource`
lost a pair that reached nothing, and **all six readers follow**, which closes
`[02-extract-tkwkb-queries-tk-numeric-under-the-shared-atol]` — live since `GkTk-remedial` prompt
12. **The `ast` guard now enumerates all seventeen object types `main.py` looks up** and fails on an
unclassified class, on a site disagreeing with its class, and on a moved site count; a second guard
cross-checks the six readers against `main.py` by `config/defaults.py` constant. **Five negative
controls all fail the suite.** **No schema change**, the four Gauss orders are still 4, and **the
three published grid digests are unmoved** because the production anchor solve is **bit-identical**
at `(1e-10, 1e-8)` and `(1e-10, 1e-9)` on both production cosmologies — which is also what closes
part (ii) of `[02a-grid-digest-not-reproducible]`: the anchor's Brent guarantee is now **3.81e-08**,
*inside* the `1e-7` at which a redshift row is matched, where it used to be 3.8e-7 outside it.
`ComputeTargets` **521**, `CosmologyModels` **39**, both OK. Board items **T8** and **T10**; one
issue opened, `[05a-two-verification-scripts-still-query-the-exit-time-under-the-shared-pair]`.
...
**Prompt 06 has measured `QuadSourceIntegral` read-only (2026-09-18, README §7 D11).**
`docs/tolerance-convergence/QUADSOURCE-READONLY.md` is the measurement and `quadsource_readonly.py`
regenerates it, offline through the fixture `ComputeTargets/tests/test_quadsource_integral.py`
builds and which it imports and does not edit. **The answer is `unchanged`** (README §6.1 rule 4):
the representation floor, measured on the *realistic* flavour at **2.52e-06 to 4.91e-04** of scale,
dominates the quadrature error at `(1e-32, 1e-8)` measured on the *exact* flavour by **×3.46e+04 to
×2.15e+11**, so **the quadrature tolerance is not the limiting parameter of the source integral**
and **§7 D4 is not reopened**. That is the campaign's **third `unchanged`**, after prompt 03's
consumer spline (×631–×37,700) and prompt 04's accumulation floor (×48.4–×3.03e5), and the largest
factor of the three by four orders. **`rtol` binds and `atol` does not**, which is the reverse of
the regime `source-remediation` log 12 measured at `atol = 1e-25`: `atol` is now inert over
twenty-eight decades and `1e-8 → 1e-11` moves `total` by **×274** rather than being bit-identical
(standing note 26). Two things the prompt did not anticipate: **`analytic_rad` is computed at the
caller's own pair**, so the code's own oracle is not fixed and moves up to ×1.25e+06 more than
`total` does; and **`inventory.py`'s selection rule had to be widened** to the four Gauss-order
columns, or the regenerated §5.1 would have dropped three of this campaign's own targets. The
inventory is current — `--check` passes, §5.4 is **44 rows** rather than 39, and §§1–4 gain four
dated subsections rather than being rewritten. **Zero production files** in the diff; two issues
opened, both **on the boards of the campaigns that own the files** (§3); `ComputeTargets` **521**,
`CosmologyModels` **39**, both OK (**T11**).
**Prompt 06a has closed the campaign (2026-09-18, README §7 D11, board item T12).**
`docs/TOLERANCE-CONVERGENCE.md` is the narrative close-out — the eight targets, the parameter each
ended at, the evidence and the floor each is now limited by — and `docs/TOLERANCE-PROVENANCE.md` is
the campaign's durable deliverable README §1.2 asked for: one entry per accuracy parameter in the
pipeline, all **44 rows** of `TOLERANCE-INVENTORY.md` §5.4, each with the five provenance fields,
assembled from the eleven campaign logs and five campaign documents **without re-measuring
anything**. Eight parameters are recorded as **unestablished, in those words**, and two —
`BESSEL_ORDER_CHECK_TOL` and the Bessel phase construction budget — are correctly *not* filed there,
both being argued from a named floor rather than swept. Both of prompt 06's standing supersessions
(rtol/atol regime inversion; `analytic_rad` not a fixed oracle) are reflected in the relevant
entries; a new §3 issue records that README §6.2's `QuadSourceIntegral` row and the comments at
`config/defaults.py:163` and `QuadSourceIntegral.py:1550` still state the superseded finding and
could not be edited under this prompt's file grant. **Zero production files, zero
`docs/tolerance-convergence/*` files and zero `docs/spec/` files** in the diff; `config/defaults.py`
byte-identical; the three published grid digests unmoved; `ComputeTargets` **521**,
`CosmologyModels` **39**, both OK; `inventory.py --check` still passes. **This closes the campaign**:
every board item (T1–T15) is now ✅, and what remains is recorded as open issues on this board's §3
and in `docs/OPEN_ISSUES.md` §1.5, not as unfinished campaign work.
...
| 05a | Decouple the tolerances that are real | README §3.5a; §2 (a), (e), (g); §7 **D1** (closed 2026-09-17), **D9** | Opus | ✍️ [`05a-…`](05a-decouple-the-tolerances-that-are-real.md) | ✅ | *"Give each solver-reaching target its own tolerance"* (SHA not embedded, per the convention `prompts/background-solver-robustness` uses) | [`logs/05a-…`](logs/05a-decouple-the-tolerances-that-are-real.md) |
| 06 | `QuadSourceIntegral`, read-only | README §0.4, §3.6; §6.1 rules 1 and 4; §7 **D11** (split 2026-09-18), **D4** (open); `[09-abserr-is-a-quadrature-bound]`, `[12-handover-clamp-error-in-production]`, `[12-atol-too-loose-for-the-source-integral]` | Opus | ✍️ [`06-…`](06-quadsource-integral-read-only.md) | ✅ | *"Measure the source integral against its own floor"* (SHA not embedded, per the convention `prompts/background-solver-robustness` uses) | [`logs/06-…`](logs/06-quadsource-integral-read-only.md) |
| 06a | Close-out and the provenance note | README §1.2, §3.6a, §6.2; §7 **D11** | Opus | ✍️ [`06a-…`](06a-close-out-and-provenance.md) | ✅ | *"Write the close-out and the tolerance provenance note"* (SHA not embedded, per the convention `prompts/background-solver-robustness` uses) | [`logs/06a-…`](logs/06a-close-out-and-provenance.md) |
...
| T10 | **PLUMBING** | Every `object_get` of a retuned target carries its own parameter, with an `ast` guard whose predicate reaches all eight targets and fails on an unclassified site **Reassigned to prompt 05a by §7 D9, 2026-09-18**, with T8. After 05 the guard enumerates four targets carrying a tolerance and four carrying none, and a tolerance reappearing on one of the latter is itself a regression it should catch. | 05a | ✅ **Done, 2026-09-18.** **11 `main.py` sites** moved to their target's own constant — `wavenumber_exit_time` ×1, `TkNumericIntegration` ×5, `GkNumericIntegration` ×3 plus the `build_missing_GkSource` conditional, `QuadSourceIntegral` ×2 unchanged — matching log 05's table exactly; the conditional is preserved and only the names inside it changed. `QuadSource`'s query batch lost a pair that reached nothing (`ComputeTargets/QuadSource.py` names neither `atol` nor `rtol`, and its other two lookups never carried it). **The guard reaches further than the row asks**: `OBJECT_TYPE_TOLERANCES` enumerates **all seventeen** object types `main.py` looks up, not the eight targets — four carrying a pair, thirteen carrying none — and the predicate is no longer `endswith("Integration")`, which matched four class names and never saw `QuadSourceIntegral` because it ends in "Integral". It fails on an unclassified **class**, on a site whose pair disagrees with its class, on an unreadable tolerance and on a moved per-class site count, and it now reads `rtol` as well as `atol`. `OneLoopIntegral` is represented by **deliberate absence plus a test that says so**, so a future lookup fails the guard rather than pre-empting `[02-oneloopintegral-is-a-ninth-keyed-object-type]`. A second case, `ReaderToleranceAgreementTestCase`, cross-checks the six `extract_*.py` against `main.py` by **`config/defaults.py` constant** rather than by local name, which is what §7 of the prompt asked for and what would have caught `[02-extract-tkwkb-queries-tk-numeric-under-the-shared-atol]` the day it landed. **Five negative controls fail the suite**, two earlier ones were discarded as bad controls and the log says why. **One site is checked by inspection, not structurally**: `build_missing_GkSource`'s `object_read_batch`, whose class name is a loop variable and whose pair is a conditional `**{...}` — recorded in the module and in log 05a §9 |
| T11 | **HAND-OFF** | `QuadSourceIntegral` measured read-only and reported to `levin-refactor` / `qsi-phase-groups` **Assigned (2026-09-18) to prompt 06 by §7 D11**, with the inventory regeneration: the measurement half of what §3.6 used to be. Measured **offline**, through the fixture `ComputeTargets/tests/test_quadsource_integral.py` already builds and which 06 imports and does not edit — the two live-run drivers need Ray and a datastore, and their statistics are already in `source-remediation`'s record. The acceptance is that the *exact* and *realistic* flavours are used to measure the quadrature error and the representation floor **separately**: a sweep on the realistic flavour alone measures the two convolved and can report neither. §6.1 rule 4's `unchanged` is the expected outcome and is a result. `TOLERANCE-INVENTORY.md` is stale at `a9ad6fc` (`inventory.py --check` fails) and 06 regenerates it, including the judgement columns a re-run does not touch. | 06 | ✅ **Done, 2026-09-18.** `docs/tolerance-convergence/QUADSOURCE-READONLY.md` + `quadsource_readonly.py`: 18 fixture cases (2 `b` x 3 shapes x 3 response redshifts), **both flavours**, the `atol` axis over **twenty-eight decades**, the `rtol` axis over six, the four corners, and a second oracle (`scipy.quad` of the exact integrand) that shares none of `analytic_integral`'s machinery and agrees to **3.49e-13** at production against its own declared 7.0e-13. **The answer is `unchanged`** (README §6.1 rule 4): the representation floor, freshly measured at **2.52e-06 to 4.91e-04** of scale, dominates the quadrature error at `(1e-32, 1e-8)` by **x3.46e+04 to x2.15e+11** — the campaign's third `unchanged` and the largest factor of the three by four orders. So **§7 D4 is not reopened** and this is a hand-back. **The `rtol` question is answered the other way from log 12** (standing note 26): at `1e-32`, `atol` is inert and binds only at `1e-16` and looser, `rtol` is the only lever, and `1e-8 -> 1e-11` moves `total` by **x274** rather than being bit-identical — the regime has inverted since that measurement and nothing in the record said so. Two findings the prompt did not anticipate: `analytic_rad` is computed at the **caller's own** pair, so it is not a fixed oracle and is displaced up to **x1.25e+06** further than `total` at production; and `inventory.py`'s selection rule had to be widened to the four Gauss-order columns or §5.1 would have dropped three of this campaign's own targets. **`TOLERANCE-INVENTORY.md` is regenerated** — `--check` passes, §5.4 is **44 rows** (not 39), the judgement columns of §4 of the prompt are corrected, and §§1–4 gain four **dated** subsections rather than being rewritten. **Zero production files** in the diff, `config/defaults.py` byte-identical, `test_quadsource_integral.py` and `convergence_reference.py` unedited, the three grid digests unmoved. `ComputeTargets` **521**, `CosmologyModels` **39**, both OK. Two issues opened, both **on the owning campaigns' boards**; `[12-atol-too-loose-for-the-source-integral]` **narrowed, not closed** |
| T12 | **PROVENANCE** | `docs/TOLERANCE-PROVENANCE.md` covers **every** accuracy parameter in the pipeline — including the ones this campaign inherits and does not set, and the ones nobody has ever chosen — with value, choosing measurement and its grid generation, competing floor, cost times object count, and citation (README §1.2) **Assigned (2026-09-18) to prompt 06a by §7 D11**, with the close-out document. The coverage checklist is `TOLERANCE-INVENTORY.md` §5.4's **44 rows** as prompt 06 left them — **not** the "39 plus the six constants 05a shipped" that README §3.6a and this row previously said, because 06's regeneration put those six *among* the 44 and adding them again double-counts (corrected 2026-09-18 when 06a was written). §1.2's closing rule is the acceptance condition diligence cannot satisfy: where a constant's provenance cannot be established from the record, the note says so **in those words**. | 06a | ✅ **Done, 2026-09-18.** `docs/TOLERANCE-PROVENANCE.md` covers all 44 rows of `TOLERANCE-INVENTORY.md` §5.4 under the inventory's own seven group letters, each with the five README §1.2 fields, assembled from the eleven campaign logs and the five campaign documents without re-measuring anything. Eight parameters are recorded as **unestablished in those words** — `DEFAULT_ABS_TOLERANCE`, `DEFAULT_REL_TOLERANCE`, both `DEFAULT_HEXIT_ABS_TOLERANCE` and `DEFAULT_GK_NUMERIC_ABS_TOLERANCE` (inert and unchosen), `DEFAULT_HEXIT_TOLERANCE`, `find_phase_extremum`'s pair, `DEFAULT_LEVIN_THRESHOLD`, `DEFAULT_LEVIN_MAX_DEPTH` and `limit = 100` — while `BESSEL_ORDER_CHECK_TOL` and the Bessel phase budget are correctly **not** filed there, both being argued from a named floor in their own comments. `docs/TOLERANCE-CONVERGENCE.md` is the companion close-out: the eight targets (three `unchanged`, two `changed`, one `drop`, one read-only `unchanged`, plus the source grid's own bit-identity acceptance), the §6.1 rule that produced each outcome, and the two corrections the campaign made to its own README (two of six shared-pair targets use the value, not one; nine keyed object types, not eight). Both of §4's supersessions (standing notes 26 and 27) are reflected in the `DEFAULT_QUADRATURE_ATOL`/`_RTOL` entries; a new §3 issue records that README §6.2, `config/defaults.py:163` and `QuadSourceIntegral.py:1550` still state the superseded regime and could not be edited under this prompt's grant. **Zero production files, zero `docs/tolerance-convergence/*` files and zero `docs/spec/` files in the diff**; `config/defaults.py` byte-identical; the three published grid digests (`3bef2c06`, `60a3205a`, `21ffc126`) unmoved; `ComputeTargets` **521**, `CosmologyModels` **39**, both OK; `inventory.py --check` still passes. |
...
- **[02-shared-atol-doubles-as-a-float-comparison-epsilon]** *(prompt 02, 2026-09-16; candidate for
  ...
  > **Narrowed by prompt 05a, 2026-09-18** (additively; **still open**). The hazard this entry
  > names did not fire, and the reason is a decision rather than luck: prompt 05a is the one prompt
  > allowed to move a parameter, and it **left `DEFAULT_ABS_TOLERANCE` at 1e-10 and
  > `DEFAULT_REL_TOLERANCE` at 1e-8** and gave the three solver-reaching targets constants of their
  > own instead. All seven sites were checked and read the same value they read before; none is in
  > the diff. **What has changed is that the constant is now *only* the epsilon**: no object type
  > keys on either name any more, so the two jobs the entry says one constant is doing have become
  > one job, and nobody has a reason to retune it for a solver. `config/defaults.py` says so where
  > the pair is defined, enumerating the seven sites by file and line and adding the signature
  > defaults of `numeric_with_phase_cut.integrate_numeric_with_phase_cut`, which this entry does not
  > list. **Why the rename was considered and rejected**: it would have had to reach three modules
  > outside prompt 05a's grant, or leave an alias, and the defect is one constant doing two jobs
  > rather than one constant having the wrong name — a rename relabels it (log 05a §2).
  > **Next step, unchanged in substance:** a prompt given `GkSource.py`,
  > `numeric_with_phase_cut.py` and `WKBtools.py` gives the seven sites an epsilon of their own,
  > named for what it compares. It is now a tidying with no hazard attached rather than a
  > precondition for a parameter move.

Opened by **prompt 05a**, 2026-09-18:

- **[05a-two-verification-scripts-still-query-the-exit-time-under-the-shared-pair]** *(prompt 05a,
  2026-09-18; unassigned)* — `docs/source-remediation-verification/analyse_greens_and_source.py:447`
  and `run_quadsource_integrals.py:140` build `atol` / `rtol` from `DEFAULT_ABS_TOLERANCE` and
  `DEFAULT_REL_TOLERANCE` and use them for a `wavenumber_exit_time` lookup — the first also for
  `GkNumericIntegration`. Prompt 05a moved `main.py` and the six `extract_*.py` readers onto
  per-target constants; **neither of these two files is in its grant** (README §0.4 puts
  `QuadSourceIntegral`'s neighbourhood out of bounds and the prompt's file list names the six
  readers only). **Impact, and it is the quiet mode rather than the loud one.** The
  `GkNumericIntegration` half is harmless: `DEFAULT_GK_NUMERIC_*` has the same value as the shared
  pair, so the key is unchanged. The exit-time half does **not** miss, because that target's lookup
  is an inequality (`[02-wavenumber-exit-time-tolerance-is-an-inequality-key]`) returning the
  loosest row at least as tight as the request: a script asking for `1e-8` is served the new
  `1e-9` row and then reports the **stored** pair rather than the requested one. So the scripts keep
  working and their provenance quietly changes, which is exactly the failure mode prompt 05a §5 was
  told to distinguish. **Next step:** a prompt given those two files repoints them at
  `DEFAULT_HEXIT_ABS_TOLERANCE` / `DEFAULT_HEXIT_REL_TOLERANCE`, and extends
  `ReaderToleranceAgreementTestCase`'s `EXTRACT_SCRIPTS` tuple to cover them — the cross-check is
  already written and is generic over a file list. Evidence: log 05a, observation 1.
...
- `ComputeTargets/QuadSourceIntegral.py:1550` still says "the pipeline supplies
  ...
  prompt 12. A stale comment in a file README §0.4 puts out of bounds. **Confirmed still stale by
  prompt 06, 2026-09-18**, which measured the sector and may not edit the file; the comment is now
  wrong twice over, because the `rtol` conclusion in the same paragraph was taken at `1e-25` and
  does not transfer (standing note 26).

Opened by **prompt 06**, 2026-09-18, and **filed to the campaigns that own the files** rather than
here (README §0.4; prompt 06 §5). This board records only that they exist and where:

- **[06-quadrature-atol-is-inert-and-rtol-is-the-binding-half]** — opened on
  [`prompts/levin-refactor`](../levin-refactor/IMPLEMENTATION_STATE.md) §3, indexed at
  `docs/OPEN_ISSUES.md` §1.3. The measurement is
  `docs/tolerance-convergence/QUADSOURCE-READONLY.md` §§4–6.
- **[06-analytic-rad-is-computed-at-the-callers-tolerance]** — opened on
  [`prompts/qsi-phase-groups`](../qsi-phase-groups/IMPLEMENTATION_STATE.md) §3, indexed at
  `docs/OPEN_ISSUES.md` §1.2. The measurement is `QUADSOURCE-READONLY.md` §2.1.

Narrowed by **prompt 06**, 2026-09-18, on another campaign's board:
**[12-atol-too-loose-for-the-source-integral]** on
[`prompts/source-remediation`](../source-remediation/IMPLEMENTATION_STATE.md) §3 — **narrowed, not
closed**, additively, per §5 rule 7. The production decision it asked for was taken
(`DEFAULT_QUADRATURE_ATOL = 1e-32`) and the regime has inverted since; what stays open is the
live-run census, which an offline prompt cannot answer.

Opened by **prompt 06a**, 2026-09-18:

- **[06a-readme-and-config-comments-state-the-superseded-quadrature-rtol-regime]** *(prompt 06a,
  2026-09-18; unassigned — three sites, for whoever next touches any one of them)* — standing note
  26 records that `source-remediation` log 12's "`rtol` does not bind" was measured at
  `atol = 1e-25` and does not transfer to the shipped `atol = 1e-32`, where `rtol` is the only lever
  and three decades move `total` by ×274 (`QUADSOURCE-READONLY.md` §§4–6, prompt 06). Three places in
  the tree still state the superseded finding and none of them was in prompt 06a's file grant:
  **README §6.2**'s `QuadSourceIntegral` row ("`rtol` confirmed non-binding (1e-8 → 1e-11
  bit-identical on 159 items)"), the comment at **`config/defaults.py:163`**, and the comment at
  **`ComputeTargets/QuadSourceIntegral.py:1550`** ("the pipeline supplies
  `DEFAULT_QUADRATURE_ATOL = 1e-25`" — already known stale by the constant's own value, and now
  stale twice over, since the `rtol` conclusion in the same paragraph does not transfer either).
  **Impact:** cosmetic — the *value* is unaffected and `docs/TOLERANCE-PROVENANCE.md`'s
  `DEFAULT_QUADRATURE_RTOL` entry records the current finding — but a reader of any of the three
  sites who does not also read the provenance note will quote a bit-identity that no longer holds
  on this tree. **Next step:** a prompt given any of the three files corrects its comment to cite
  the inverted regime (`QUADSOURCE-READONLY.md` §§4–6) rather than `source-remediation` log 12's
  figure alone; README §6.2 is this campaign's own charter table and its rule 8 freeze does not
  bar a later prompt from amending prose that is now factually wrong, only from moving a parameter.
  Evidence: `docs/TOLERANCE-CONVERGENCE.md` §6; `docs/TOLERANCE-PROVENANCE.md`, the
  `DEFAULT_QUADRATURE_RTOL` entry.

---

## 4. Resolved issues

Closed by **prompt 05a**, 2026-09-18:
...
- **[02-extract-tkwkb-queries-tk-numeric-under-the-shared-atol]** *(prompt 02, 2026-09-16;
  ...
  **both** `TkNumericIntegration` and `TkWKBIntegration`. `main.py` writes every
  ...
  > **CLOSED by prompt 05a, 2026-09-18.** `extract_TkWKB_data.py` now builds
  > `Tk_numeric_atol` / `Tk_numeric_rtol` from `DEFAULT_TK_NUMERIC_ABS_TOLERANCE` and
  > `DEFAULT_TK_NUMERIC_REL_TOLERANCE` and passes them to the `TkNumericIntegration` lookup alone,
  > which is the pair `main.py` writes that target under. The prompt's instruction to check the
  > other five for the same fault was carried out: **no other reader queries
  > `TkNumericIntegration` at all**, so the split constant's absence hurt only this one — but
  > `extract_Gk_data.py` and `extract_GkWKB_data.py` were in the same *shape* on
  > `GkNumericIntegration`, a shared constant standing in for a target-specific one and harmless
  > only because the two happened to share a value. Both now name their own constant, so the
  > coincidence is no longer load-bearing. **The one-line fix is not what shipped**: a structural
  > test shipped with it. `ReaderToleranceAgreementTestCase` in
  > `ComputeTargets/tests/test_main_plumbing.py` resolves the `config/defaults.py` constant behind
  > every tolerance each reader and `main.py` pass — through their `ray.get` tuples and through
  > `**query_payload` dicts — and requires them to agree per class; it compares constants, not
  > local names, and needs no datastore. It fails on a reverted `extract_TkWKB_data.py`, which is
  > negative control 4 of log 05a §7. Still not verified by *running* the script, which needs a
  > datastore: what is established is that the key now matches.
...
- **[02a-grid-digest-not-reproducible]** *(2026-09-17; assigned — **T6** / prompt 03 for the
  ...
  **Explicitly not prompt 02a's** (README §4, §7 D6): 02a's acceptance is bit-identity with the
  ...
  > **CLOSED by prompt 05a, 2026-09-18** — part (ii), part (i) having been delivered by prompt 03a.
  >
  > **What closes it is the ordering of two tolerances, and nothing else.** The defect is that the
  > anchor was pinned only to `xtol + rtol*|u| = 3.8e-7` relative while the redshift table matches a
  > row at `DEFAULT_REDSHIFT_RELATIVE_PRECISION = 1e-7` — so the grid tag could turn over while
  > every row it is built from was re-matched and reused. At `DEFAULT_HEXIT_REL_TOLERANCE = 1e-9`
  > the Brent guarantee at the largest production $|u| = 38.04$ is **3.81e-08**, *inside* the row
  > match. The anchor is now pinned finer than the precision at which its consequences are
  > compared, which is what "one design tolerance applied to the row match and the digest together"
  > amounts to once the digest itself may not be touched (prompt 05a §6, README §0.5).
  >
  > **Measured, not argued, and it is stronger than the bound.** The production anchor solve
  > `_solve_horizon_exit(cosmology, 3e8/Mpc, −5)` returns the **same double** at `(1e-10, 1e-8)` and
  > at `(1e-10, 1e-9)` on both production cosmologies — `2.0636395964154036e+16` on LambdaCDM,
  > `3.30033444460513e+16` on QCD, relative move **0.000e+00** — so the tightening displaced no
  > anchor and moved no grid. All three published digests are unmoved (`3bef2c06`, `60a3205a`,
  > `21ffc126`), which is also acceptance 10 of prompt 05a.
  >
  > **What is not claimed.** The digest is still bit-exact and still turns over under a 1e-14
  > relative perturbation of the anchor; the entry's "quantise the digest" and "digest the
  > determining data" options were both **not** taken, the second because
  > `[03a-qcd-v2-grid-sample-count-is-not-reproducible]` showed the integer subdivision vector
  > itself moves on QCD. That issue **stays open** and is where the residue lives. The caution
  > above about `xtol` also stands: at `xtol = 1e-10` the pair cannot pin the anchor better than
  > 1e-10 relative, so any future design tolerance below that requires
  > `DEFAULT_HEXIT_ABS_TOLERANCE` to move with its partner, and its comment says so.
...
26. **`source-remediation` log 12's "`rtol` does not bind" was measured at `atol = 1e-25` and does
    not transfer to this tree** (prompt 06, 2026-09-18). The constant has read `1e-32` since that
    campaign's own prompt 12, and at that value the two halves have swapped roles: `atol` is inert
    across twenty-eight decades and binds only at `1e-16` and looser, while `rtol` is the only
    parameter that moves the source integral — about a decade of quadrature error per decade of
    tolerance, and `1e-8 → 1e-11` moves `total` by ×274 rather than leaving 159 items
    bit-identical. **Nothing follows for the value**: §6.1 rule 4 gives `unchanged`, the
    representation floor dominating by ×3.46e+04 to ×2.15e+11. But a prompt quoting the
    bit-identity — from `config/defaults.py:163`, from `QuadSourceIntegral.py:1550`, or from
    `TOLERANCE-INVENTORY.md` as prompt 02 left it — is quoting a superseded regime.
    `QUADSOURCE-READONLY.md` §§4–6.

27. **`analytic_rad` is not a fixed oracle** (prompt 06, 2026-09-18). It is computed inside
    `evaluate_QuadSource_integral` at **the caller's own** `atol` and `rtol`
    (`QuadSourceIntegral.py:884-897`), so a stored `analytic_rad` depends on its row's
    `atol_serial` and `rtol_serial`, and a residual `|total − analytic_rad|` taken from one run
    compares two quantities that moved together. At the production pair it sits up to **×1.25e+06**
    further from its converged value than `total` does. Any prompt sweeping tolerances against it
    must hold it at a reference pair, as `quadsource_readonly.py` does.
    `[06-analytic-rad-is-computed-at-the-callers-tolerance]`, on `qsi-phase-groups`' board.
