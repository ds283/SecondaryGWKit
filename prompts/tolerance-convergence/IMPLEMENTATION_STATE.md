...
**Last updated:** 2026-09-18 · **Status: in progress — 6 / 6 measured, plus the insertions 02a, 04b, 05b and 05a; **04b has landed the block prompt 04 stopped short of writing**, **05 has landed the schema change**, **05b has made the recorded order the order that was used** and **05a has decoupled the tolerances that are real**; the campaign's production work is finished and only the read-only measurement and the close-out are left. **§7 D11 split §3.6 on 2026-09-18**: **06** takes the `QuadSourceIntegral` measurement and the inventory regeneration, **T11**, and is written; **06a** takes the close-out document and `docs/TOLERANCE-PROVENANCE.md`, **T12**, and is held until 06 lands.**
...
| 06 | `QuadSourceIntegral`, read-only | README §0.4, §3.6; §6.1 rules 1 and 4; §7 **D11** (split 2026-09-18), **D4** (open); `[09-abserr-is-a-quadrature-bound]`, `[12-handover-clamp-error-in-production]`, `[12-atol-too-loose-for-the-source-integral]` | Opus | ✍️ [`06-…`](06-quadsource-integral-read-only.md) | ⬜ | | |
...
| T11 | **HAND-OFF** | `QuadSourceIntegral` measured read-only and reported to `levin-refactor` / `qsi-phase-groups` **Assigned (2026-09-18) to prompt 06 by §7 D11**, with the inventory regeneration: the measurement half of what §3.6 used to be. Measured **offline**, through the fixture `ComputeTargets/tests/test_quadsource_integral.py` already builds and which 06 imports and does not edit — the two live-run drivers need Ray and a datastore, and their statistics are already in `source-remediation`'s record. The acceptance is that the *exact* and *realistic* flavours are used to measure the quadrature error and the representation floor **separately**: a sweep on the realistic flavour alone measures the two convolved and can report neither. §6.1 rule 4's `unchanged` is the expected outcome and is a result. `TOLERANCE-INVENTORY.md` is stale at `a9ad6fc` (`inventory.py --check` fails) and 06 regenerates it, including the judgement columns a re-run does not touch. | 06 | ⬜ |
