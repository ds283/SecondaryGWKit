...
- **[01-the-eq22-figure-is-eq22s-own-rounding-not-the-head]** *(opened 2026-09-18 by prompt 01)* —
  ...
  **Resolution (2026-09-18, follow-up commit after the campaign closed).** Re-measured independently
  and recorded additively as `KOHRI-TERADA-ORACLE.md` §7.2, generated by the new
  `docs/radiation-oracle/eq22_rounding.py`. That script uses a 50-digit `mpmath` eq. (22) sharing no
  code with either transcription, and a 50-digit head, on all nine $b=0$ cases. It reproduces
  §7.1's 3.18e-10 exactly with the audit's own double-precision eq. (22) and head. With both at 50
  digits the worst $\lvert N+9/8\rvert$ is **3.67e-13**, the same order as §7's 3.09e-13 against the
  quadrature. So the pipeline's agreement with eq. (22) on q-smooth is ~1e-13, tighter than the
  ~1e-11 estimated above. The head's error, declared or actual, is $\le1.4\times10^{-18}$ of $I$.
  `kt_verification.I_RD`'s own error at the three q-smooth points is 2.8e-10, 1.4e-10 and 4.5e-11.
  The audit's §1 table carries a dated pointer to §7.2; the generated sections §4–§7.1 are
  unchanged. Campaign README §1 carries the same pointer. Prompt 01, the orchestrator prompt and
  item R1's description are left as the records of what was asked. **Closed.**
