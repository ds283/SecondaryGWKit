...
SELECT vs.serial                 AS serial,
       ...
       rs.z                      AS z,
       vs.value                  AS value
FROM TkNumericValue vs
         INNER JOIN TkNumericIntegration ig ON ig.serial = vs.integration_serial
...
SELECT vs.serial                 AS serial,
       ...
       ss.z                      AS z_source,
       vs.value                  AS value
FROM GkNumericValue vs
         INNER JOIN GkNumericIntegration ig ON ig.serial = vs.integration_serial
         INNER JOIN wavenumber_exit_time kes ON kes.serial = ig.wavenumber_exit_serial
         INNER JOIN wavenumber ks ON ks.serial = kes.wavenumber_serial
         INNER JOIN redshift rs ON rs.serial = vs.z_serial
