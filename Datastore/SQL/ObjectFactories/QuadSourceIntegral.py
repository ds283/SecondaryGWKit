...
class sqla_QuadSourceIntegral_factory(SQLAFactoryBase):
    ...
    def build(payload, conn, table, inserter, tables, inserters):
        ...
        try:
            ...
            print(
                f"!! QuadSourceIntegral.build(): multiple results found when querying for QuadSource (q={q.k.k_inv_Mpc:.5g}/Mpc, store_id={q.store_id} | r={r.k.k_inv_Mpc:.5g}/Mpc, store_id={r.store_id})"
            )
            raise e
...
class sqla_QuadSourceIntegralValue_factory(SQLAFactoryBase):
    ...
    def read_batch(payload, conn, table, tables):
        ...
        parent_table = tables["GkWKBIntegration"]
        redshift_table = tables["redshift"]
