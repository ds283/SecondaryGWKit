...
11. **Statements about $h_{ij}$ normalisation relative to the literature.** MAIN 11 p.4: "$h^{\rm us}_{ij} = h^{\rm them}_{ij}/2$" (Kohri & Terada); MAIN 11 p.10: Adshead et al.'s "$h_{ij}$ is half the one used here". Transcribed as written; the transcriber has not checked whether these two statements are mutually consistent given the papers' respective conventions.
   **Kohri–Terada half closed 2026-09-18 — checked, not only transcribed.** At $b=0$ the code's source integral equals $-\tfrac98\,k_{\rm phys}^{-2}$ times Kohri & Terada's closed form (their eq. 22, less the head $0\to\bar x_{\rm min}$), and $-\tfrac98 = -\tfrac12\times\tfrac94$ with $\tfrac94 = 1/c^2$ at $b=0$ (spec 03 §0.1) and the sign the orientation convention of spec 04 §0(3). The remaining $\tfrac12$ is $h^{\rm us}_{ij} = h^{\rm them}_{ij}/2$, as MAIN 11 p.4 states. Measured on the nine $b=0$ fixture cases to $\lvert N+\tfrac98\rvert \le 1.1\times10^{-10}$ (`docs/radiation-oracle/KOHRI-TERADA-ORACLE.md` §1.1, §7; `prompts/radiation-oracle/logs/01-kohri-terada-radiation-oracle.md` §3.1) and pinned in the suite by `ComputeTargets.tests.test_kohri_terada_oracle.TestPipeline.test_total_is_minus_nine_eighths_of_I_RD`, which fails on all nine cases if the $\tfrac12$ is dropped (same log, §3.2 row K1). The check is at $w=1/3$ only. The $\tfrac12$ is a normalisation of $h_{ij}$ and carries no $w$-dependence, but the general-$w$ prefactor $N(b)$ is predicted, not measured (`[01-general-w-normalisation-is-predicted-not-measured]`, radiation-oracle board). **Still open:** the Adshead et al. statement (MAIN 11 p.10), and with it whether the two statements are mutually consistent.
