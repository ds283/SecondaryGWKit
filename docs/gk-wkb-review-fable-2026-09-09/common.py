...
def zarr(zs):
    return redshift_array([redshift(i, float(z)) for i, z in enumerate(zs)])


def exact_theta(k, zi, z):
    return mp.mpf(k) * (1 / (1 + mp.mpf(zi)) - 1 / (1 + mp.mpf(float(z))))


def phase_err(div, mod, k, zi, zs):
    return np.array(
        [
            float(mp.mpf(int(d)) * 2 * mp.pi + mp.mpf(float(m)) - exact_theta(k, zi, z))
            for d, m, z in zip(div, mod, zs)
        ]
    )
