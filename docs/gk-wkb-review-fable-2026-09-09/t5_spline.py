...
def build(k, s_lo, s_hi, per_decade, chunklog):
    s = np.geomspace(s_lo, s_hi, int(np.log10(s_hi / s_lo) * per_decade) + 1)
    z = s - 1
...
def probe(k, per_decade, chunklog):
    ...
    logs = np.log(s)
    fine = np.concatenate(
        [np.linspace(logs[i], logs[i + 1], 11)[:-1] for i in range(len(s) - 1)]
    )
    zf = np.exp(fine) - 1
    exact = np.array(
        [
            float(mp.mpf(k) * (mp.exp(-mp.mpf(float(u))) - 1) - base * 2 * mp.pi)
            for u in fine
        ]
    )
    ...
    err = raw - exact
    nint = len(s) - 1
    interior = slice(3 * 10, (nint - 3) * 10)
    # chunk switching: which chunk is used at each fine point
    which = np.array(
        [
            spl._chunk_list.index(
                next(
                    c
                    for c, sp in spl._splines.items()
                    if sp is spl._match_chunk(float(u))[0]
                )
            )
            for u in fine
        ]
    ...
    for i in switches:
        ...
        a = spl._splines[spl._chunk_list[which[i]]]
        b = spl._splines[spl._chunk_list[which[i + 1]]]
        ta = a.raw_theta(raw_x=np.exp(u) - 1, log_x=u, warn_unsafe=False)
        tb = b.raw_theta(raw_x=np.exp(u) - 1, log_x=u, warn_unsafe=False)
        da = a.theta_deriv(
            raw_x=np.exp(u) - 1, log_x=u, log_derivative=True, warn_unsafe=False
        )
        db = b.theta_deriv(
            raw_x=np.exp(u) - 1, log_x=u, log_derivative=True, warn_unsafe=False
        )
    ...
    knot_err = max(
        abs(
            float(
                spl.raw_theta(float(zz), x_is_log=False) - (float(t) - base * 2 * np.pi)
            )
        )
        for zz, t in zip(z, th)
    )
    print(
        f"k={k:.0e} {per_decade:5.0f}/dec chunklog={str(chunklog):>4}: chunks={spl.num_chunks} | interior max|err|={np.max(np.abs(err[interior])):.2e} rad, incl. ends={np.max(np.abs(err)):.2e} | predicted h^4 x_max/384={ (np.log(10)/per_decade)**4*k/10/384:.1e} | max spline ordinate={ymax:.3g} rad (global span {abs(float(th[-1]-th[0])):.3g}) | knot residual={knot_err:.2e} | switches={len(switches)} "
        + "; ".join(
            f"at s={sz:.4g}: dtheta={dj:.2e} rad, dtheta'/theta'={dd:.1e}"
            for sz, dj, dd in jumps
        )
    )
