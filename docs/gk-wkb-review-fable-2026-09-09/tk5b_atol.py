...
T0, Tp0 = compute_analytic_T(k, 1 / 3, 1 / s_i), compute_analytic_Tprime(
    k, 1 / 3, 1 / s_i, s_i**2
)
for atol, rtol in [(1e-10, 1e-8), (1e-13, 1e-8), (1e-16, 1e-8), (1e-20, 1e-8)]:
    ...
    print(
        f"exact init, (atol,rtol)=({atol:.0e},{rtol:.0e}): nfev={out['data'].RHS_evaluations:6d}, max|dT|/env = {max(e):.2e}; |T| at stop = {abs(out['stop_value']):.2e}"
    )
