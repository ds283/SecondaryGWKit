...
import ComputeTargets.TkWKBIntegration

TK = sys.modules["ComputeTargets.TkWKBIntegration"]
...
def w_mp(z):
    s = 1 + mp.mpf(z)
    return (Or * s / 3) / (Om + Or * s)


def lead_mp(z_lo, z_hi, k):
    f = (
        lambda u: mp.mpf(k)
        * mp.sqrt(w_mp(mp.exp(u) - 1))
        / H_mp(mp.exp(u) - 1)
        * mp.exp(u)
    )
...
zi = brentq(lambda z: log(xof(z)) - 3, 1.0, z_exit)
zs = grid((1 + zi) / 10 ** (1 / 100) - 1, 0.1, 100)
proxy = NS(get=lambda: model)
kmock = NS(k=NS(k=k, k_inv_Mpc=k, store_id=0))
...
nf = out["friction_data"].RHS_evaluations
meta = out["metadata"]
print(
    f"k={k:.0e}/Mpc: z_init=z_e3={zi:.4g}, {len(zs)} source samples to z=0.1; stage1 nfev={n1} ({meta.get('phase_cycle_events')} resets), stage2 nfev={n2}, friction nfev={nf}; Qmin={meta.get('stage_2_smallest_Q')}; time {dt:.1f}s"
)
...
def resid(z):
    def f(v):
        zz = exp(v) - 1
        kcH = k * sqrt(cosmo.wPerturbations(zz)) / H(zz)
        o2 = Tk_omegaEff_sq(model, k, zz)
...
print(
    f"{'z':>10} {'x_T':>9} {'theta_ref':>16} {'phase err':>10} {'rel':>8} {'friction F':>11} {'dF':>9}"
)
worst = 0
worstF = 0
for i in idx:
    ...
    got = mp.mpf(int(out["theta_div_2pi_sample"][i])) * 2 * mp.pi + mp.mpf(
        float(out["theta_mod_2pi_sample"][i])
    )
    e = float(got - ref)
    worst = max(worst, abs(e))
    Fref = -F_mp(z, zi)
    dF = float(mp.mpf(float(out["friction_sample"][i])) - Fref)
    worstF = max(worstF, abs(dF))
    print(
        f"{z:10.4g} {k*sqrt(cosmo.wPerturbations(z))*(1+z)/H(z):9.3g} {float(ref):16.8g} {e:10.3e} {abs(e)/abs(float(ref)):8.1e} {float(Fref):11.5f} {dF:9.2e}"
    )
print(
    f"max |phase err| = {worst:.3e} rad; max |dF| (= relative amplitude error from friction) = {worstF:.2e}"
)
