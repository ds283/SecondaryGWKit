...
def sim(k, x_r, mode, rebase):
    s_e3, s_e4 = k / exp(3), k / exp(4)
    s_lim = sqrt(s_e3 * s_e4)
    src = np.geomspace(s_lim, s_e3, int(np.log10(s_e3 / s_lim) * 100) + 1)
    full = np.geomspace(1.0, s_e3, int(np.log10(s_e3) * 100) + 1)
    resp = full[::-1][::12][::-1]
    theta = lambda a, b: k * (1 / a - 1 / b)
    s_r = resp[np.argmin(abs(resp - k / x_r))]
    recs = []
    for s_s in src:
        x_s = k / s_s
        p4 = -(exp(4) - x_s)
        off = pi / 2 if mode == "extremum" else 0.0
        n = floor((-p4 - off) / pi)
        th_i = -(off + n * pi)
        while th_i > p4:
            th_i -= pi
        s_i = 1 / (1 / s_s - th_i / k)
        G_i = (s_s**2 / k) * sin(th_i)
        Gp_i = np.cos(th_i) * s_s**2 / s_i**2
        om_i = k / s_i**2
        ...
        if rebase:
            d2, m2 = shift_theta_sample(d, m, delta)
        else:
            sh = [wrap_theta(mm + delta) for mm in m]
            d2 = [dd + s for dd, (s, _) in zip(d, sh)]
            m2 = [mm for (_, mm) in sh]
        j = int(np.argmin(abs(rs - s_r)))
        recs.append(
            (int(d2[j]), float(m2[j]), theta(s_s, s_r), wrap_theta(m[0] + delta)[0])
        )
    last_mod = last_div = None
    sub = blk = None
    rect = []
    for div, mod, ex, b in recs:
        if last_mod is None:
            ...
            if TWO_PI * div + mod > TWO_PI * last_div + last_mod:
                dm = mod - last_mod
                jump = min(
                    [(fabs(dm), 0), (fabs(dm + TWO_PI), 1), (fabs(dm - TWO_PI), -1)],
                    key=lambda x: x[0],
                )[1]
                rd = blk + jump
                sub = div - rd
                ...
                rd = div - sub
            blk = rd
        last_mod, last_div = mod, div
        rect.append(TWO_PI * rd + mod)
...
for mode in ["extremum", "zero"]:
    for x_r in [1e2, 1e3, 1e4]:
        ...
        for key in tot:
            ...
            print(
                f"stop@{mode:8s} x_r={x_r:.0e} rebase={key[:-1]:5s}: {n} objects over 15 k values, base=+1: {bp}, base=-1: {bm}, 2pi-kinks after rectifier: {kinks}"
            )
