...
def run(k, x_init, per_decade, rtol, atol, force_stage1=False, z_lo=0.1):
    ...
    if force_stage1:
        W.DEFAULT_OMEGA_WKB_SQ_MAX = 1e300
    ...
    return dict(
        ...
        n1=n1,
        n2=n2,
        resets=meta.get("phase_cycle_events", 0),
        Qmin=meta.get("stage_2_smallest_Q"),
        nsamp=len(zs),
        dt=dt,
    )
...
print(
    f"{'span':>8} {'grid/dec':>8} {'rtol':>7} {'atol':>7} {'stage1?':>7} {'max|dtheta|':>12} {'rel':>8} {'nfev1':>7} {'nfev2':>6} {'resets':>6} {'Qmin':>8} {'t[s]':>5}"
)
for k in [1.1e5, 1.1e7, 1.1e9]:
    for per_decade in [100 / 12, 100]:
        for rtol, atol in [(1e-8, 1e-10), (1e-11, 1e-13), (5e-14, 1e-16)]:
            ...
            print(
                f"{r['span']:8.2g} {per_decade:8.3g} {rtol:7.0e} {atol:7.0e} {'stg2':>7} {r['maxerr']:12.3e} {r['maxerr']/r['span']:8.1e} {r['n1']:7d} {r['n2']:6d} {r['resets']:6d} {r['Qmin']:8.3g} {r['dt']:5.1f}"
            )
    if k < 1e8:
        for rtol, atol in [(1e-8, 1e-10), (5e-14, 1e-16)]:
            r = run(k, 30.0, 100 / 12, rtol, atol, force_stage1=True)
            print(
                f"{r['span']:8.2g} {100/12:8.3g} {rtol:7.0e} {atol:7.0e} {'stg1':>7} {r['maxerr']:12.3e} {r['maxerr']/r['span']:8.1e} {r['n1']:7d} {r['n2']:6d} {r['resets']:6d} {'-':>8} {r['dt']:5.1f}"
            )
