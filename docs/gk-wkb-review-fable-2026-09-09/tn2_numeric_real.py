...
def tau_mp(z):  # conformal time from the big bang, int_z^inf dz/H
    ...
    return mp.quad(
        ...
        [
            mp.log(1 + mp.mpf(z)),
            mp.log(1 + mp.mpf(z)) + 5,
            mp.log(1 + mp.mpf(z)) + 15,
            mp.log(1 + mp.mpf(z)) + 40,
        ],
    )
...
for k in [1e5, 3e8]:
    ...
    z_e6 = brentq(lambda z: log(xof(z)) - 6, 1.0, z_exit)
    ...
    res = {}
    for label, patch in [
        ("with diagnostic", None),
        ("without diagnostic", lambda m, kk, z: 1.0),
    ]:
        saved = GN.Gk_omegaEff_sq
        if patch:
            GN.Gk_omegaEff_sq = patch
        ...
        out = run(
            ...
            delta_logz=0.01,
            mode="stop",
            stop_search_window_z_begin=z_e3,
            stop_search_window_z_end=z_e6,
            task_label="tn2",
            object_label="G",
        )
