...
zi = brentq(
    lambda z: Tk_omegaEff_sq(model, k, z) - 1e6, 0.1, 1e9
)  # stage-2 start: omega^2 = 1e6
om = lambda z: sqrt(Tk_omegaEff_sq(model, k, z))
om_i = om(zi)


def rhs(u, y):
    z = zi - u
    return [-om(z) / om_i / (1 + u) - y[0] / (1 + u)]
...
def ref(z):
    lead = mp.quad(
        lambda v: mp.mpf(k)
        * mp.sqrt(w_mp(mp.exp(v) - 1))
        / H_mp(mp.exp(v) - 1)
        * mp.exp(v),
        [mp.log(1 + mp.mpf(z)), mp.log(1 + mp.mpf(zi))],
    )
    ...
    return -(
        lead
        + mp.mpf(
            quad(f, log(1 + z), log(1 + zi), limit=800, epsrel=1e-10, epsabs=1e-14)[0]
        )
    )


print(
    f"k={k:.0e}: stage-2 start z={zi:.4g}, {len(sol.t)-1} accepted steps, nfev={sol.nfev}"
)
...
for u, z in ends:
    e = float(mp.mpf(om_i * (1 + u)) * mp.mpf(float(sol.sol(u)[0])) - ref(z))
    print(f"{u:10.6f} {z:10.4g} {'step end':>9} {e:16.3e}")
zg = np.geomspace(1.1, 3.0, 5) - 1
for z in zg:
    u = zi - z
    e = float(mp.mpf(om_i * (1 + u)) * mp.mpf(float(sol.sol(u)[0])) - ref(z))
    print(f"{u:10.6f} {z:10.4g} {'grid pt':>9} {e:16.3e}")
...
for z in zg[[0, -1]]:
    u = zi - z
    e = float(mp.mpf(float(st.sol(u)[0])) - ref(z))
    print(
        f"{u:10.6f} {z:10.4g} {'theta-ODE':>9} {e:16.3e}   (direct theta, dense output; nfev={st.nfev})"
    )
