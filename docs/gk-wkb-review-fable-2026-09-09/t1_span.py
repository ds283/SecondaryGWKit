...
m.functions = ModelFunctions(
    ...
    d2_epsilon_dz2=lambda z: 2.0 * cosmo.d2_lnH_dz2(z)
    + (1.0 + z) * cosmo.d3_lnH_dz3(z),
    wBackground=cosmo.wBackground,
    wPerturbations=cosmo.wPerturbations,
    tau=None,
    T_photon=cosmo.T_photon,
    d_lnH_dz=cosmo.d_lnH_dz,
    d2_lnH_dz2=cosmo.d2_lnH_dz2,
    d3_lnH_dz3=cosmo.d3_lnH_dz3,
    d_wPerturbations_dz=cosmo.d_wPerturbations_dz,
...
def C_of(z):
    eps = m.functions.epsilon(z)
    epsP = m.functions.d_epsilon_dz(z)
    s = 1 + z
    return -epsP / 2.0 / s + (1.5 * eps - eps * eps / 4.0 - 2.0) / (s * s)


print(
    f"{'k[1/Mpc]':>9} {'z_exit':>9} {'z_e3':>9} {'x(z=0.1)':>10} {'theta_lead(e3->0.1)':>19} {'residual':>10} {'eps*theta':>10} {'1e-8*theta':>10}"
)
for k in [1e5, 1e6, 1e7, 1e8, 3e8]:
    ...
    def f_res(u):
        z = exp(u) - 1
        kH = k / H(z)
        om2 = Gk_omegaEff_sq(m, k, z)
        return (om2 - kH * kH) / (sqrt(om2) + kH) * exp(u)

    res, _ = quad(f_res, log(1 + ZEND), log(1 + z_e3), limit=500, epsrel=1e-10)
    print(
        f"{k:9.3g} {z_exit:9.3g} {z_e3:9.3g} {x_of(k, ZEND):10.3g} {lead:19.6g} {res:10.4g} {2.2e-16*lead:10.2g} {1e-8*lead:10.2g}"
    )
...
for xi in [20, 30, 100, 1000]:
    ...
    print(
        f"k=1e6: residual phase from x={xi} to z=0.1: {res:.4g} rad ; C at that z = {C_of(z_a):.3g}, 1/x={1/xi:.3g}"
    )
