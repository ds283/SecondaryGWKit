...
z_exit = brentq(lambda z: np.log(xof(z)), 1.0, 1e18)
z_e3 = brentq(lambda z: np.log(xof(z)) - 3.0, 1.0, z_exit)
zi = z_e3
...
def resid(z):
    f = lambda u: (lambda zz, kH, o2: (o2 - kH * kH) / (np.sqrt(o2) + kH) * np.exp(u))(
        np.exp(u) - 1, k / cosmo.Hubble(np.exp(u) - 1), om2(model, k, np.exp(u) - 1)
    )
    r, _ = quad(f, np.log(1 + z), np.log(1 + zi), limit=500, epsrel=1e-10, epsabs=1e-14)
    return r


idx = list(range(0, len(zs), max(1, len(zs) // 12))) + [len(zs) - 1]
print(
    f"k={k:.3g}/Mpc rtol={rtol:.0e} atol={atol:.0e}: z_e3={zi:.4g}, {len(zs)} response samples, stage1 nfev={n1}, stage2 nfev={n2}, resets={meta.get('phase_cycle_events')}, stage1->2 at z={meta.get('stage1_z_terminate')}, Qmin={meta.get('stage_2_smallest_Q')}, time {dt:.1f}s"
)
