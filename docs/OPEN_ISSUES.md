...
**Last updated:** 2026-09-17 · **74 open** across ten campaigns.
...
| `[01-density-criterion-imposed-outside-the-wkb-region]` | tolerance-convergence | `source_grid_spacing_profile` imposes the fourth-derivative spline criterion over `residual_node_range`'s band, which reaches **1.5–2.1 e-folds outside the horizon** — ~5 e-folds beyond the phase spline it protects. 69% of the samples v2 adds on QCD lie above horizon crossing for the smallest production $k$. **Narrowed 2026-09-17, and its evidence is now measured** — prompt 02a made the grid buildable without touching the band. The guarded-node census is the direct measure of the overreach: **53 nodes (Gk 34, Tk 19) of 136,453 band-node evaluations on QCD at its own anchor, 53 of the 100 (k, sector) cases and exactly one node each; zero at either published anchor.** So the band reaches just past the edge of the region at a single lattice node rather than through a stretch of breakdown. Unjustified rather than wrong; the decision remains **T7**. |
