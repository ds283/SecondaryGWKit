...
| Issue | Board | Hook |
|---|---|---|
| `[05-rho-gauss-order-reaches-the-residual-table-through-a-default-argument]` | tolerance-convergence | Prompt 05 put `rho_gauss_order` in both WKB lookup keys on the invariant that **no keyword, payload key or default** can make a row record an order it was not computed at. `phase_residual.build_phase_residual` / `cached_phase_residual` take `order: int = RHO_GAUSS_ORDER`, a **def-time snapshot**, so a caller passing `order=6` would get a table at 6 under a key that says 4. No production caller does — asserted by `ast` over every production module — so the impact today is zero and the hazard is silent: nothing would fail. Fix is to make `order` required and have `WKB_phase_function` pass the constant explicitly. Neither file was in prompt 05's list. |
