...
**Last updated:** 2026-09-13 · **53 open** across six campaigns.
...
prompts plus seven follow-ups — 14, 15, 16, 17, 18, 19, 20 — of which 18 are executed;
prompts 20 and 13 remain, in that order). `[07-phase-spline-chunking-precision]` was closed WONTFIX on
...
| `[18-numeric-solver-not-in-lookup-key]` | GkTk-remedial | The numeric lookup keys carry no record of *how* the integration was done — `GkNumericIntegration.py:221-227` and `TkNumericIntegration.py:225-231` filter on the wavenumber, model, `atol` and `rtol` only — so pre- and post-prompt-18 QCD rows are indistinguishable by key while differing by up to 2.82e-4 of the envelope, and prompt 19's per-sector policy moved QCD `TkNumericIntegration` a further 1.61e-4, again invisibly. **Diagnosis corrected by the user 2026-09-13:** adding `solver_serial` to the queries — the originally recorded remedy — is a **no-op**, because the solver is hard-coded (`method="DOP853"`, a constant label, and `main.py`'s `solvers` menu never indexed), so every numeric row points at the same `IntegrationSolver` serial; no factory anywhere filters on one, and five compute targets store one. What is unkeyed is the *configuration*, `break_point_kind`. **Assigned (2026-09-13): prompt 20** — key it beside `atol`/`rtol` as prompt 12 did for the $T_k$ `atol`, leave the solver as provenance, no migration for old datastores, and audit the other four solver columns report-only. Runs before prompt 13. |
