...
| Issue | Board | Hook |
|---|---|---|
| `[05-rho-gauss-order-reaches-the-residual-table-through-a-default-argument]` | tolerance-convergence | Prompt 05 put the four Gauss orders in the lookup key on the invariant that **no keyword, payload key or default** can make a row record an order it was not computed at. Two paths break it. `phase_residual.build_phase_residual` / `cached_phase_residual` take `order: int = RHO_GAUSS_ORDER`, so a caller passing `order=6` gets a table at 6 under a key saying 4; no production caller does, asserted by `ast`. **Widened 2026-09-18, and the second half is larger**: the *rehydration* path is wrong for all four orders, not just `rho` — `BackgroundModel`'s three `_build_*_primitive` methods reassemble a stored model's cumulative tables at the **current** module constant while the row's own order columns are selected by `build()` and never passed to the constructor, and both WKB classes' properties return the constant whatever the row said. Masked only because `build()` filters on that constant, so a served row always happens to carry it. **Assigned (2026-09-18): prompt 05b**, board item **T15**, running before 05a under §7 **D10** — the user's decision that this is a correctness issue and must be made correct: set up, computed, persisted, rehydrated. |
