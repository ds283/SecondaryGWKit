...
print(
    f"    worst |div*2pi+mod - theta|/|theta| = {worst:.3e}; range violations metric {worst_range:.3e}"
)
...
print(
    f"    reconstructed theta+shift up to constant offset: spread of offset/2pi = "
    f"{np.ptp(offset)/TWO_PI:.3e}, offset/2pi = {offset[0]/TWO_PI:.6g}"
)
print(
    f"    max |sin(new_mod) - sin(theta+shift)| = "
