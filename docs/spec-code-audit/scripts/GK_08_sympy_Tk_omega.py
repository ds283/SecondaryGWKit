...
C = (ep / 2 * (3 * w - eps + 1) + sp.Rational(3, 2) * wp * (eps - 3 * (1 + w))) / opz**2
D = (
    -(3 * (1 + eps) * (1 + w) - eps * (3 + eps / 2) - sp.Rational(9, 2) * (1 + w) ** 2)
    / opz**3
)
code = A + B + C + D

print(
    "R43 omega_eff^2: code form matches spec by inspection (c_s^2 = w, 3 c_s c_s' = 3w'/2)"
)
print(
    "WKB_Tk.Tk_d_ln_omegaEff_dz numerator - d(omega^2)/dz =", sp.simplify(code - true)
)

# and the pre-641bb51 form, with (3/2)(1+w) in place of 3(1+w) in the w' coefficient
C_old = (
    ep / 2 * (3 * w - eps + 1)
    + sp.Rational(3, 2) * wp * (eps - sp.Rational(3, 2) * (1 + w))
) / opz**2
