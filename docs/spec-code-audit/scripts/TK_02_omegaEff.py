...
R24_spec = (
    sp.Rational(1, 2) * (epsf - 3 * (1 + w)) / opz**2
    - sp.Rational(1, 2) * (sp.Derivative(epsf, z) - 3 * sp.Derivative(w, z)) / opz
)
...
print(
    "R27 definition - R27/R29 compact:",
    sp.simplify(sp.expand((omega_def - omega_compact).doit())),
)
...
print(
    "code Tk_omegaEff_sq - R29:",
    sp.simplify(sp.expand((code_omega_sq - omega_compact).doit())),
)
...
A = wp * (k_code / H) ** 2
B = (sp.Rational(3, 2) * wpp - epp / 2 - 2 * epsf * w * (k_code / H) ** 2) / opz
C = (
